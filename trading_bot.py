...
IDIOMA = os.environ.get("IDIOMA", "es").lower()  # "es" o "en"

# ============ Mercados y sus horarios locales de operación ============
MERCADOS = [
    {"nombre_es": "Sídney (ASX)",      "nombre_en": "Sydney (ASX)",      "tz": "Australia/Sydney", "apertura": (10, 0), "cierre": (16, 0),  "bandera": "🇦🇺"},
    {"nombre_es": "Tokio (TSE)",       "nombre_en": "Tokyo (TSE)",       "tz": "Asia/Tokyo",       "apertura": (9, 0),  "cierre": (15, 0),  "bandera": "🇯🇵"},
    {"nombre_es": "Londres (LSE)",     "nombre_en": "London (LSE)",      "tz": "Europe/London",    "apertura": (8, 0),  "cierre": (16, 30), "bandera": "🇬🇧"},
    {"nombre_es": "Nueva York (NYSE)", "nombre_en": "New York (NYSE)",   "tz": "America/New_York", "apertura": (9, 30), "cierre": (16, 0),  "bandera": "🇺🇸"},
]

TEXTOS = {
    "es": {
        "abre": "▸ Abre en 5 minutos",
        "cierra": "▸ Cierra ahora",
        "overlap_inicia": "▸ Inicia overlap",
        "overlap_finaliza": "▸ Finaliza overlap",
        "gold": "Oro",
        "sin_precios": "(precios no disponibles ahora mismo)",
    },
    "en": {
...
}[IDIOMA if IDIOMA in ("es", "en") else "es"]


def nombre_mercado(m):
    return m["nombre_en"] if IDIOMA == "en" else m["nombre_es"]
...
def obtener_precios():
    """Devuelve (precio_oro, precio_btc). Si falla la API, devuelve (None, None)."""
    try:
...
def main():
    ...
    etiqueta_overlap = "London + New York" if IDIOMA == "en" else "Londres + Nueva York"
